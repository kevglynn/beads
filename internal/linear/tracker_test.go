...
// TestBatchPush_SkipsUnchangedIssue_WithStructuredFields verifies that BatchPush
// still skips unchanged issues when Description is already pre-formatted by the
// engine hook and structured fields are also present on the local issue.
func TestBatchPush_SkipsUnchangedIssue_WithStructuredFields(t *testing.T) {
	var updateCalled bool
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		var req GraphQLRequest
		...
		case strings.Contains(req.Query, "IssueByIdentifier"):
			...
			json.NewEncoder(w).Encode(issueByIdentifierResp(
				"remote-uuid", "TEAM-1", "My Issue", remoteDescription, 0,
				"state-open", "Backlog", "backlog",
			))
		case strings.Contains(req.Query, "issueUpdate"):
			updateCalled = true
	...
	local := &types.Issue{
		...
		AcceptanceCriteria: "Meets condition",
	...
	if updateCalled {
		t.Error("UpdateIssue was called for an unchanged structured-field issue; expected it to be skipped")
	}
	if len(result.Skipped) != 1 || result.Skipped[0] != "local-structured" {
		t.Errorf("Skipped = %v, want [local-structured]", result.Skipped)
	}
	if len(result.Updated) != 0 {
		t.Errorf("Updated = %v, want []", result.Updated)
	}
